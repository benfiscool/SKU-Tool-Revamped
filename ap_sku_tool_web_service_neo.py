...
def upload_file(local_path, drive_filename):
    ...
    results = service.files().list(q=f"name='{drive_filename}' and trashed=false",
...
def api_latest_db_file_id():
    ...
    results = service.files().list(
        q=f"name contains '{prefix}' and name contains '{suffix}' and trashed=false",
        spaces='drive',
...
def api_pull_latest_db():
    ...
    try:
        ...
        if not matching:
            # DON'T show error dialog - just return error status
...
def manual_download():
    service = get_drive_service()
    # List files for user to pick
    results = service.files().list(q="trashed=false", spaces='drive', fields="files(id, name)").execute()
    files = results.get('files', [])
    if not files:
        messagebox.showinfo("Download", "No files found in Drive.")
        return
...
def api_delete():
    ...
    try:
        if not file_id and filename:
            ...
            files = results.get('files', [])
            if not files:
                return jsonify({"status": "error", "message": f"No file named {filename} found."}), 404
            file_id = files[0]['id']
        if not file_id:
